...
void show_helper(splay_tree_node_t * node){
  if(node){
    show_helper(node->left);
    printf("The node is %d\n",node->key);
    show_helper(node->right);
  }
}

void splay_tree_show(splay_tree_t *sp){
  show_helper(sp->root);
}

static inline void
rotate_left (splay_tree_node_t **pp, splay_tree_node_t *p, splay_tree_node_t *n)
{
  splay_tree_node_t *tmp;
  tmp = n->right;
  n->right = p;
  p->left = tmp;
  *pp = n;
...
static inline void
rotate_right (splay_tree_node_t **pp, splay_tree_node_t *p, splay_tree_node_t *n)
{
  splay_tree_node_t *tmp;
  tmp = n->left;
  n->left = p;
...
static void
splay_tree_splay (splay_tree_t *sp, int key)
{
  if (sp->root == NULL)
    return;
...
splay_tree_insert (splay_tree_t *sp, splay_tree_node_t *node)
{
  int comparison = 0;
...
attribute_hidden void
splay_tree_remove (splay_tree_t *sp, int key)
{
  splay_tree_splay (sp, key);

  if (sp->root && splay_compare (sp->root->key, key) == 0)
    {
      splay_tree_node_t *left, *right;
...
splay_tree_lookup (splay_tree_t *sp, int key)
{
  splay_tree_splay (sp, key);
...
// static void
// splay_tree_foreach_internal (splay_tree_node_t *node, splay_tree_callback func,
// 			     void *data)
// {
//   if (!node)
//     return;
...
// attribute_hidden void
// splay_tree_foreach (splay_tree_t *sp, splay_tree_callback func, void *data)
// {
