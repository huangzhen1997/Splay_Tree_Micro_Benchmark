...
static inline int splay_compare (int x, int y)
{
  if (x==y) {
    return 0;
  } else if (x<=y) {
...
typedef void (*splay_tree_callback) (int, void *);

extern int splay_tree_lookup (splay_tree_t *, int);
extern void splay_tree_insert (splay_tree_t*, splay_tree_node_t *);
extern void splay_tree_remove (splay_tree_t *, int);
extern void splay_tree_show(splay_tree_t *sp);
